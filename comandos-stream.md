# 🤖 Comandos BotDelEstadio - Hablemos de Pavadas

## 🎵 **Audios** | 🎮 **Gaming** | 💰 **Puntitos** | 🎪 **Minijuegos** | 📺 **Canal** | 🛠️ **Utilidades**

### 🎵 **Comandos de Audio**
`!holis` `!helldiver` `!cuervo` `!zazaraza` `!piripipi` `!dark` `!quiereme` `!sacrilegioso` `!sadsong` `!boca` `!yeahbaby` `!wansaia82` `!presta` `!distinta` `!sega` `!play` `!mario` `!zelda` `!alert` `!ernesto` `!horny` `!yamete`
...
---
*💡 **Tip**: Algunos comandos tienen alias. Ej: `!puntos` = `!puntitos` = `!consulta` | `!dato` = `!datos` = `!tip` = `!tips` | `!escupir` = `!spit` = `!garzo` | Para admins: `!puntito [usuario]` `!bienvenida [usuario]` `!top [n]` `!record`*